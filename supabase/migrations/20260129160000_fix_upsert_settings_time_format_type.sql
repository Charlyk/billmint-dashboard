-- Fix type mismatch in upsert_user_settings function
-- The COALESCE was comparing TEXT with time_format enum
-- Also add separate functions for profile and app settings

-- Fix the main function (still used for full updates)
...
-- Function for profile/billing defaults (hourly rate, currency)
...
-- Function for app preferences (time format, week starts on, timer settings)
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION update_billing_defaults(UUID, TEXT, NUMERIC) TO authenticated;
GRANT EXECUTE ON FUNCTION update_billing_defaults(UUID, TEXT, NUMERIC) TO service_role;

GRANT EXECUTE ON FUNCTION update_app_settings(UUID, TEXT, SMALLINT, NUMERIC) TO authenticated;
GRANT EXECUTE ON FUNCTION update_app_settings(UUID, TEXT, SMALLINT, NUMERIC) TO service_role;
