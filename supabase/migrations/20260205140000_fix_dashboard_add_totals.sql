-- Fix get_dashboard_data to include total_invoices and total_time_entries
-- while preserving all existing functionality (overdue_invoices, currency handling, enum casts)
                ...
                'overdue_invoices', (
                    SELECT json_build_object(
                        'count', (
                            SELECT COUNT(*)::INTEGER
                            FROM invoices
                            WHERE user_id = p_user_id AND status = 'overdue'::invoice_status
                        ),
                        'amounts', COALESCE((
                            SELECT json_agg(json_build_object('currency', currency, 'amount', ROUND(amount::NUMERIC, 2)))
                            FROM (
                        ...
                        'invoices', COALESCE((
                            SELECT json_agg(json_build_object(
                                ...
                                'currency', i.currency,
                                'due_date', i.due_date,
                                'days_overdue', (CURRENT_DATE - i.due_date::DATE)
                            ) ORDER BY i.due_date ASC)
                            FROM invoices i
                            LEFT JOIN clients c ON c.id = i.client_id
                            WHERE i.user_id = p_user_id
                                AND i.status = 'overdue'::invoice_status
                        ), '[]'::json)
                    )
                )
            )
