# Requirements Archive: v1.1 Observability Hardening
...
- [x] **LOG-06**: All API route handlers use withLogging wrapper for correlation ID propagation and request lifecycle logging
- [x] **LOG-07**: withLogging wrapper logs request method, path, status code, and response time for every API request

### Analytics Cleanup

- [x] **CLN-01**: Unused `first_*` funnel event helpers are removed from `events.ts`
- [x] **CLN-02**: PostHog "First time event" filter approach is documented as deliberate decision (not tech debt)
...
*Last updated: 2026-02-11 — All v1.1 requirements complete (8/8)*
